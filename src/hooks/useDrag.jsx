...
export default function useDrag({
  ...
  useEffect(() => {
    console.log('hahaha' ,document.getElementsByClassName(dragger));// null
  },[])

  useEffect(() => {
    if(!document.querySelector('.hahaha')) return;
    if (!dragger) return;
    if (!draggerBox) return;
    if (!container) return;
    dragger =
      typeof dragger === "string" ? document.getElementsByClassName(dragger) : dragger; // 根据传入的值类型，去查找使用元素
    draggerBox =
      typeof draggerBox === "string"
        ? document.getElementsByClassName(draggerBox)
        : draggerBox;
    container =
    ...
    console.log('container',document.getElementsByClassName(container), document.querySelector('.hahaha'))
    const {
      left: containerL,
      top: containerT,
      right: containerR,
